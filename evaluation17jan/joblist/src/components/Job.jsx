...
export const Job=()=>{
    ...
    <button onClick={()=>{setTodo(todo.filter(word => word.name===name))}} className="border-solid border-2 border-sky-500 w-20 m-2">Filter</button>
    <button onClick={()=>{setTodo(todo.sort((a,b)=>b.salary-a.salary))}} className="border-solid border-2 border-sky-500 w-20 m-2">l to h</button>
    <button onClick={()=>{setTodo(todo.sort((a,b)=>a.salary-b.salary))}} className="border-solid border-2 border-sky-500 w-20 m-2">h to l</button>
    {todo.map((e)=>(
...
<button disabled={page==1} onClick={()=>{setPage(page-1)}} className="border-solid border-2 border-sky-500 w-1/2 m-2">Prev</button>
<button onClick={()=>{setPage(page+1)}} className="border-solid border-2 border-sky-500 w-1/2 m-2">Next</button>
</div>
</div>);}